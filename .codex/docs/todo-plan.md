...
| :-- | :-- | :-- | :-- |
| DOC-001 | done | Create the `Orchestrator Agent` system prompt for project-boundary analysis, document scoping, and target directory planning. | `.codex/agents/orchestrator-agent.md` |
| DOC-002 | done | Create `doc-scope-rule` so agents must determine project boundary before choosing the document set. | `.codex/rules/doc-scope.md` |
| DOC-003 | done | Create the `plan-doc-structure` skill for first-pass document tree and document-list planning from the PRD. | `.codex/skills/plan-doc-structure/` |
| DOC-004 | done | Create the `/plan-docs` command as the document-planning entry point. | `.codex/commands/plan-docs.md` |
| DOC-005 | done | Create the summary-sync rule so every document-generation step updates durable progress tracking. | `.codex/rules/summary-sync.md` |
| DOC-006 | done | Create the `Architect Agent` system prompt. | `.codex/agents/architect-agent.md` |
| DOC-007 | done | Create the `generate-global` skill. | `.codex/skills/generate-global/` |
| DOC-008 | done | Create the `/generate-global` command. | `.codex/commands/generate-global.md` |
| DOC-009 | done | Create the `Domain Planner Agent` system prompt. | `.codex/agents/domain-planner-agent.md` |
| DOC-010 | done | Create the `plan-domains` skill. | `.codex/skills/plan-domains/` |
| DOC-011 | done | Create the `/plan-domains` command. | `.codex/commands/plan-domains.md` |
| DOC-012 | done | Create the `Domain Spec Agent` system prompt. | `.codex/agents/domain-spec-agent.md` |
| DOC-013 | done | Create the `generate-domain-spec` skill. | `.codex/skills/generate-domain-spec/` |
| DOC-014 | done | Create the `/generate-domain <name>` command. | `.codex/commands/generate-domain.md` |
| DOC-015 | done | Create the `Test Design Agent` system prompt. | `.codex/agents/test-design-agent.md` |
| DOC-016 | done | Create the `generate-test-design` skill. | `.codex/skills/generate-test-design/` |
| DOC-017 | done | Create the `/generate-tests` command. | `.codex/commands/generate-tests.md` |
| DOC-018 | done | Create the `Acceptance Agent` system prompt. | `.codex/agents/acceptance-agent.md` |
| DOC-019 | done | Create the `generate-acceptance` skill. | `.codex/skills/generate-acceptance/` |
| DOC-020 | done | Create the `/generate-acceptance` command. | `.codex/commands/generate-acceptance.md` |
