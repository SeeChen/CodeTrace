...
| :-- | :-- | :-- | :-- |
| DOC-001 | done | Create the `Orchestrator Agent` system prompt for project-boundary analysis, document scoping, and target directory planning. | `.qoder/agents/orchestrator-agent.md` |
| DOC-002 | done | Create `doc-scope-rule` so agents must determine project boundary before choosing the document set. | `.github/rules/doc-scope.md` + `.qoder/rules/doc-scope.md` |
| DOC-003 | done | Create the `plan-doc-structure` skill for first-pass document tree and document-list planning from the PRD. | `.qoder/skills/plan-doc-structure/` |
| DOC-004 | done | Create the `/plan-docs` command as the document-planning entry point. | `.qoder/commands/plan-docs.md` |
| DOC-005 | done | Create the summary-sync rule so every document-generation step updates durable progress tracking. | `.github/rules/summary-sync.md` + `.qoder/rules/summary-sync.md` |
| DOC-006 | done | Create the `Architect Agent` system prompt. | `.qoder/agents/architect-agent.md` |
| DOC-007 | done | Create the `generate-global` skill. | `.qoder/skills/generate-global/` |
| DOC-008 | done | Create the `/generate-global` command. | `.qoder/commands/generate-global.md` |
| DOC-009 | done | Create the `Domain Planner Agent` system prompt. | `.qoder/agents/domain-planner-agent.md` |
| DOC-010 | done | Create the `plan-domains` skill. | `.qoder/skills/plan-domains/` |
| DOC-011 | done | Create the `/plan-domains` command. | `.qoder/commands/plan-domains.md` |
| DOC-012 | done | Create the `Domain Spec Agent` system prompt. | `.qoder/agents/domain-spec-agent.md` |
| DOC-013 | done | Create the `generate-domain-spec` skill. | `.qoder/skills/generate-domain-spec/` |
| DOC-014 | done | Create the `/generate-domain <name>` command. | `.qoder/commands/generate-domain.md` |
| DOC-015 | done | Create the `Test Design Agent` system prompt. | `.qoder/agents/test-design-agent.md` |
| DOC-016 | done | Create the `generate-test-design` skill. | `.qoder/skills/generate-test-design/` |
| DOC-017 | done | Create the `/generate-tests` command. | `.qoder/commands/generate-tests.md` |
| DOC-018 | done | Create the `Acceptance Agent` system prompt. | `.qoder/agents/acceptance-agent.md` |
| DOC-019 | done | Create the `generate-acceptance` skill. | `.qoder/skills/generate-acceptance/` |
| DOC-020 | done | Create the `/generate-acceptance` command. | `.qoder/commands/generate-acceptance.md` |
