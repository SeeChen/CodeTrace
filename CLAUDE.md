...
- The **library this repo generates** is a lightweight, local-first, zero-dependency Python function tracer. Its PRD (`docs/PRD.md`) is the single product-level source of truth.
- The **real deliverable** is the pipeline itself (under `.claude/`). `src/` and `tests/` are generated by running the pipeline, proving the workflow end to end.
...
- `docs/Pipeline-Components.md` — per-agent and per-skill reference; `docs/diagrams/` — PlantUML flow + sequence
